...
package o_project_compiler.symboltable;
...
public class Tab extends rs.etf.pp1.symboltable.Tab {
    ...
    public static void dump(DumpSymbolTableVisitor SymbolTableVisitor, Logger logger) {
        if (SymbolTableVisitor == null) {
            SymbolTableVisitor = new DumpSymbolTableVisitor(true);
        }
        for (Scope s = currentScope; s != null; s = s.getOuter()) {
            s.accept(SymbolTableVisitor);
        }
        logger.info("Symbol table:\n" + SymbolTableVisitor.getOutput());
    }
